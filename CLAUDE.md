...
if (!sdk.oauth.isAuthorized()) {
  sdk.emit('hubble-home-assistant:data', { error: 'Not authorized' });
  return;
}
...
// Your connector MUST emit to that exact topic: sdk.emit('hubble-home-assistant:data', ...)
// If you emit to a different topic (e.g. ':state'), pass args explicitly:
//   useConnectorData<MyData>('hubble-home-assistant', 'hubble-home-assistant:state')
